...
case class DgraphCluster(name: String, version: String) {
  ...
  def alterSchema(): Unit = {
    ...
    @tailrec
    def attempt(no: Int, limit: Int): Unit = {
      ...
      if (hasErrors(response)) {
        assert(no < limit, "Retry limit exceeded, giving up")
        Thread.sleep(1000)
        attempt(no + 1, limit)
      }
    }
  ...
  def insertData(): Map[String, Long] = {
    ...
    @tailrec
    def attempt(no: Int, limit: Int): Map[String, Long] = {
  ...
  def hasErrors(response: Response): Boolean = {
    val json = new Gson().fromJson(response.text(), classOf[JsonObject])
    val errors =
